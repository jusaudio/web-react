...
export default class GoogleSheets {
  ...
  public async appendMultipleRows<T>(appendValues: T[], startRow: number): Promise<any> {
    await this.connect();
    await this.getHeaders();
    
    // input = [{ "A": 1, "B": 2 }, { "A": 11, "B": 22 }];
    // -> [[1, 2], [11, 22]];
    // Left off notes: Implement multiple append values on given startRow,
    // possibly using batchUpdate, append, batchAppend
    
  }

  public async clearRowsWithRange(from: number = 2, to: number = 1000): Promise<any> {
    await this.connect();
    await this.getHeaders();

    const range = `${this.sheetName}!${from}:${to}`;
    console.info(`Will clear rows range: ${range}`);
    const clearResults: any = await this.connection.spreadsheets.values.clear({
      spreadsheetId: this.spreadsheetId,
      range
    });
    return clearResults;
